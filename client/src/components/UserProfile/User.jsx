...
export const User = ({ email, gravatar, contactNumber }) => {
  ...
  return (
    <section className="flex items-center justify-end gap-3 md:gap-6">
      ...
      {isUserInfoModalOpen && <section className={`${isUserInfoModalOpen ? "bg-gray-300 border border-gray-200 rounded-lg absolute top-[19rem] right-0 w-full md:max-w-lg xl:right-[25%] dark:bg-black dark:border-white" : "hidden"}`}><section className="relative p-4"><UserInfo email={email} contactNumber={contactNumber} onClickHandler={() => onChangeToggleUserInfoModalHandler(false)} /></section></section>}
    </section>
  );
};
